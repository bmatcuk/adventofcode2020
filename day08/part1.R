...
# convert argument to int, add a "reached" column defaulting to false
ops <- read_lines("input.txt") %>%
  enframe(name = NULL) %>%
  extract(value, c("opcode", "argument"), "(\\w+) ([+-]\\d+)") %>%
  mutate(
...
repeat {
  ...
  ops[ip,]$reached = T
  if (ops[ip,]$opcode == "acc") acc <- acc + ops[ip,]$argument
  if (ops[ip,]$opcode == "jmp") ip <- ip + ops[ip,]$argument
  else ip <- ip + 1
}
