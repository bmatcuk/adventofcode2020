...
while (length(player1) > 0 && length(player2) > 0) {
  len <- min(length(player1), length(player2))
  hand1 <- head(player1, len)
  hand2 <- head(player2, len)
  winner1 <- which(hand1 > hand2)
  player1 <- c(tail(player1, -len), rbind(hand1[winner1], hand2[winner1]))
  player2 <- c(tail(player2, -len), rbind(hand2[-winner1], hand1[-winner1]))
}

sum(apply(cbind(player1, rev(seq_along(player1))), 1, prod)) +
  sum(apply(cbind(player2, rev(seq_along(player2))), 1, prod))
