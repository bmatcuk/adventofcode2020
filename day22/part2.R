...
# To defend your honor as a Raft Captain, you challenge the small crab to a
# game of Recursive Combat.
#
# Recursive Combat still starts by splitting the cards into two decks (you
# offer to play with the same starting decks as before - it's only fair). Then,
...
# card, this would occur if player 1 has at least 3 cards left and player 2 has
# at least 7 cards left, not counting the 3 and 7 cards that were drawn.)
#
...
blanks <- which(input == "")
player1 <- as.integer(input[2:(blanks[1] - 1)])
player2 <- as.integer(input[(blanks[1] + 2):length(input)])

play <- function(player1, player2) {
  prev_player1 <- c()
  prev_player2 <- c()
  while (length(player1) > 0 && length(player2) > 0) {
    if (any(unlist(lapply(prev_player1, identical, player1))) && any(unlist(lapply(prev_player2, identical, player2)))) {
      return(list(player1, c()))
    }

    prev_player1 <- append(prev_player1, list(player1))
    prev_player2 <- append(prev_player2, list(player2))

    hand1 <- player1[[1]]
    hand2 <- player2[[1]]
    player1 <- tail(player1, -1)
    player2 <- tail(player2, -1)
    handsize1 <- length(player1)
    handsize2 <- length(player2)
    winner1 <- hand1 > hand2
    if (hand1 <= handsize1 && hand2 <= handsize2) {
      results <- play(player1[1:hand1], player2[1:hand2])
      winner1 <- length(results[[1]]) > 0
    }
    if (winner1) {
      player1 <- c(player1, hand1, hand2)
    } else {
      player2 <- c(player2, hand2, hand1)
    }
  }
  list(player1, player2)
}

sum(map_dbl(play(player1, player2), ~ sum(apply(cbind(., rev(seq_along(.))), 1, prod))))
