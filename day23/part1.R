...
for (i in seq(100)) {
  try <- if (input[1] == 1) 9 else input[1] - 1
  move_to <- which(input == try)
  while (2 <= move_to && move_to <= 4) {
    try <- if (try == 1) 9 else try - 1
    move_to <- which(input == try)
  }
  if (move_to == 1) {
    input <- c(tail(input, -1), input[1])
  } else {
    input <- c(input[5:move_to], input[2:4], tail(input, -move_to), input[1])
  }
}

idx <- which(input == 1)
paste0(c(tail(input, -idx), head(input, idx - 1)), collapse = "")
