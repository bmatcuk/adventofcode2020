...
# first line is the earliest we can leave
earliest <- as.integer(input[1])

# the second line is comma separated - load into tibble
# split it on the comma into lines
# remove lines with "x"
# convert to ints and calculate how long we'll wait
# find the minimum wait
# calculate result
enframe(input[2], name = NULL) %>%
  separate_rows(value, sep = ",") %>%
  filter(value != "x") %>%
  mutate(
    value = as.integer(value),
    wait = value * ceiling(earliest / value) - earliest
  ) %>%
  slice_min(wait) %>%
  summarize(value * wait)
