...
rules <- enframe(input[1:(blanks[1] - 1)], name = NULL) %>%
  ...
  pivot_longer(!name, names_to = c(".value"), names_pattern = "(min|max)")

# build a list of valid numbers
valid <- summarize(rules, valid =
    reduce2(
      min,
      max,
...
# separate them into one column
# sum invalid numbers
enframe(input[(blanks[2] + 2):length(input)], name = NULL) %>%
  separate_rows(value, sep = ",", convert = T) %>%
  { .$value } %>%
  reduce(.f = function(acc, v) {
    if (is.na(valid[v])) {
      acc + v
    } else {
      acc
