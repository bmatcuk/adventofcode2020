...
# rating of 0.
#
# Since you have some time tokill, you might as well test all of your adapters.
...
# read file and sort
# add a row that's 3 greater than max
# add a column that diffs with previous row
# group by diff
# count per diff
# drop rows where diff wasn't 1 or 3
# calculate product of those counts
t <- read_table2("input.txt", col_names = F) %>% arrange(X1)
add_row(t, X1 = last(t$X1) + 3) %>%
  mutate(diff = X1 - lag(X1, default = 0)) %>%
  group_by(diff) %>%
  count() %>%
